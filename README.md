...
GPL-2 payload for niimath's optional `-spm_coreg` and `-spm_deface` commands — a C port of SPM's `spm_coreg` rigid-body coregistration (John Ashburner, Wellcome Centre for Human Neuroimaging). Used as the `src/GPL` git submodule of [niimath](https://github.com/rordenlab/niimath).
